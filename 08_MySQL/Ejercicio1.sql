...
select id_emp, nombre, cargo_emp from empleados;
...
select nombre, (sal_emp+comision_emp+500)  from empleados
where id_depto = 3000
order by nombre;
...
*/
select nombre, sal_emp, comision_emp, (sal_emp+comision_emp) from empleados
...
‘Mantenimiento.*/
select * from empleados
where cargo_emp = 'Vendedor' or cargo_emp = 'Investigador' or cargo_emp = 'Mecanico';
/*20. Ahora obtener el contrario, los nombres de los departamentos que no sean “Ventas” ni
“Investigación” ni ‘Mantenimiento.*/
select * from empleados
where cargo_emp != 'Vendedor' and cargo_emp != 'Investigador' and cargo_emp != 'Mecanico';
...
select count(id_emp) , id_depto as departamento from empleados
group by id_depto
